...
- Colour schemes (user request): Cenovka (default) plus pastel ones, mostly pink: Marcipán, Cukrová vata, Pudr, Broskev, a mint Máta and a light blue Pomněnka. Each has a dark variant that follows the phone. Chosen in Nastavení → Vzhled, stored per phone and in backups. The home-screen icon stays yellow.
- Styles (user request): two looks of the same app, chosen in Nastavení → Vzhled separately from the colours, so either works with any scheme. Hranatý is the printed price-tag look; Kreslený draws the same app by hand on squared paper — uneven outlines drawn twice, crayon fills, torn rail edges, wobbling icons, and names, labels and sentences set in Patrick Hand. Amounts stay in Archivo in both, because the numbers must read at a glance. The style is stored per phone and in backups.
